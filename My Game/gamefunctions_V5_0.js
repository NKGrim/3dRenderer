...
//** Revision History
//***************************************************************************
//** Version 5.0 - 4/21/2023
//** - Put in logic to allow ScrollingBackground to work correctly in the y-
//**   direction
//** 
//** Version 5.1 - 11/8/2023
//** - Added in logic to correctly track x and y values for mouse relative to
//**   the canvas
//** - Commented out getColor function due to browser security updates
//**   (function is now obsolete due to not able to work correctly)
...
function GameMaster(){
    ...
    this.moveObjectWithBackground = function(o){
        o.x += this.bgDX;
        o.y += this.bgDY;
        // Moves object from left side to right side.
        if (o.x < o.useWidth*-1 && this.bgDX < 0){
            o.x = canvas.width + this.bgDX;
        }
        // Moves object from right side to left side.
        if (o.x > canvas.width && this.bgDX > 0){
            o.x = 0 - o.useWidth - this.bgDX;
        }
        // Moves object from top side to bottom side.
        if (o.y < o.useHeight*-1 && this.bgDY < 0){
            o.y = canvas.height + this.bgDY;
        }
        // Move object from bottom side to top side.
        if (o.y > canvas.height && this.bgDY > 0){
            o.y = 0 - o.useHeight - this.bgDY;
        }
    }
...
/* This function is no longer useable due to browser security updates that doesn't allow
   image data to be gathered using getImageData function.
...
function KeysPresses(){
    ...
    this.processMouseMove = function(e) {
        ...
        this.movementX = Math.abs(e.movementX) > 3 ? e.movementX : 0; //seems to be off by a bit so create deadzone
        this.movementY = Math.abs(e.movementY) > 3 ? e.movementY : 0;

        this.deltaX = Math.abs(e.movementX) > 2 ? e.movementX : 0; //movement of mouse in 1 frame
        this.deltaY = Math.abs(e.movementY) > 2  ? e.movementY : 0;
    }
